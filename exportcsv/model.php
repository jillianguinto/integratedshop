<?php
	...
	{ 
		if(count($results) > 0 ){  
			...
			$filename  = $_SERVER['DOCUMENT_ROOT'].'/exportcsv/csv/'.$name.'.csv';
	...
	{
		$ftphost		= "119.81.194.35";
		$ftpuser		= "unilabdbm";
		$ftppassword 	= "{jXWevLQ(?+0";		
	...
	function import_to_dbm($name)
	{
		...
		$ch = curl_init('http://dbm.unilab.com.ph/dbm/api/integimport/' . $name . '/');
