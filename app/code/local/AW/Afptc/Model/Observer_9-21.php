...
class AW_Afptc_Model_Observer extends Varien_Object
{
    public function checkoutCartSaveAfter($observer)
    {
        ...
        foreach ($rulesCollection as $ruleModel) {
            ...
            $hascouponcode = $ruleModel->getCouponCode();
            $afptccode = Mage::getSingleton('core/session')->getafptccode();
            // $logmo = "Coupmo ".$hascouponcode." ".$ruleModel->getId()." ".$afptccode;
            // Mage::log($logmo, null, 'checkoutCartSaveAfter.log');
...
Mage::log($sku, null, 'eskeyu.log');
                       ...
                       if($sku_value ==  $_cartItem->getProduct()->getsku())
                        {
                            ...
                            Mage::log('ruleid' . $ruleModel->getId(), null, 'product_id--.log');
                ...
                if($freeitemqty == $y_qty)
                {

                    $canAdd = false;
                }

                Mage::log('free' . $freeitemqty, null, 'product_id--.log');
                Mage::log('cart' . $y_qty, null, 'product_id--.log');
                ...
                if($canAdd == true):    
                    ...
                    //check if has coupon code
                    // if($hascouponcode == ""){
                    

                    $cart->addProduct($product, array('qty' => $y_qty));

                    // }else{
                    //     if($hascouponcode == $afptccode){
                    //         $cart->addProduct($product, array('qty' => $y_qty));
                    //     }
                    // }
    ...
    function removeFreeitem($productId)
    {
        ...
        // Mage::log($productId, null, 'removeFreeitem1.log');
