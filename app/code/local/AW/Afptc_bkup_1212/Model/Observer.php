...
class AW_Afptc_Model_Observer extends Varien_Object
{
    ...
    foreach ($rulesCollection as $ruleModel) {
         ...
         $checkadd = Mage::getSingleton('core/session')->getaddfree();

        $coupon_code = $ruleModel->getcoupon_code();

if($checkadd != 0){

   if($promocoupon == $coupon_code){
        ...
        // $this->removeFreeitem($freeproduct);
                ...
                foreach ($cart->getQuote()->getAllItems() as $_cartItem) {
                    ...
                    foreach (explode(",", $sku) as $sku_value) {
                       if($sku_value ==  $_cartItem->getProduct()->getsku())
                        {
                            $y_qty  = $_cartItem->getQty(); //quantiy ng primary
                        } 
         ...
         // var_dump($ruleModel->getproduct_id());
         // die();



        //         if($y_qty >= $ruleModel->getdiscount_step()){ // dagdag
        ...
        //                 while($QtyCart >= $ruleModel->getdiscount_step()){    // dito bago
                ...
                if($_key == '_origData'):
                    ...
                    foreach ($value as $key => $_value) {
                        ...
                        if($key == 'conditions_serialized'):
                           ...
                           foreach($getsku as $key=>$_value): // loop for getting SKU conditions
                           ...
                           endforeach;  // end loop for getting SKU conditions
                           foreach (explode(",", $sku) as $sku_value) {
                                              // echo $sku_value;
                                              //   die();
