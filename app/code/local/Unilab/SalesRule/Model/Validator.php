 ...
 */
class Unilab_SalesRule_Model_Validator extends Mage_Core_Model_Abstract
{
    /**
        ...
        foreach ($this->_getRules() as $rule) {
            ...
            switch ($rule->getSimpleAction()) {
                ...
                case Mage_SalesRule_Model_Rule::BY_PERCENT_ACTION:
					...
					if($rule->getDiscountMaxAmount()) {
					
					$maxDiscount = $rule->getDiscountMaxAmount() * $_rulePct;
					$expectedDiscount = ($address->getDiscountAmount()*-1)+$discountAmount;
                    //var_dump( $expectedDiscount.$maxDiscount);
                   
					if($expectedDiscount > $maxDiscount) {
					
					    if(($address->getDiscountAmount()*-1) >= $maxDiscount) {
							$discountAmount = 0;
							
							}
						else {
						$newDiscount= ($address->getDiscountAmount())+$maxDiscount;
							$discountAmount  = $newDiscount;
							
							
							}
						//var_dump($discountAmount);//$address->getDiscountAmount()*-1);
