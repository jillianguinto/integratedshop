    ...
    {	
		...
		// {
			...
			if (in_array($host, $paramArr)) {
				...
				$hostname = str_replace("www.","", $parsehost);
			}
		...
		Mage::log($categoryid, null, 'jillian.log');
		Mage::log($storename, null, 'jillian.log');
		Mage::log($hostname, null, 'jillian.log');
		...
		if($tokenid){
			...
			Mage::getSingleton('core/session')->setHttpReferer($referer);
			Mage::getSingleton('core/session')->setStoreid($storeid);		
			Mage::getSingleton('core/session')->setStorename($storename);	
			Mage::getSingleton('core/session')->setHostname($hostname);			
