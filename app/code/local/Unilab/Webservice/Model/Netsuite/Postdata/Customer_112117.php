...

{
...

	{
		...
		$query 	= "SELECT senttoNS from customer_entity WHERE entity_id='$customerId'"; 
...

	{
		...
		try{
			...
			Mage::log($api_ResponseData, true, 'sendCustomerfrombackend_response_true_'.date("Y-m-d").'.log');

			Mage::log($postdata, true, 'sendCustomerfrombackend_post_true_'.date("Y-m-d").'.log');
