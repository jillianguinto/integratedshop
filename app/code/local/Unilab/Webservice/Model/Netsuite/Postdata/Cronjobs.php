...
class Unilab_Webservice_Model_Netsuite_Postdata_Cronjobs extends Mage_Core_Model_Abstract
{
	...
	{	
		...
		$responnseCustomer = array();
		
		$query 	= 'SELECT entity_id FROM  customer_entity WHERE sent_dbm_netsuite = 0 and website_id = 15 order by entity_id LIMIT 10'; 
		...
		foreach ($result as $value) 
		{
			...
			if($response['success'] == 1 ){
				$responnseCustomer['customerid'][] = $response['customer_id'];
	...
	public function updateCustomer()
	{
		...
		$query 	= 'SELECT entity_id FROM  customer_entity WHERE senttoNS = 1 and update_senttoNS = 0 and sent_dbm_netsuite = 1 and website_id = 15 order by entity_id LIMIT 10'; 
		$result = $this->_getConnection()->fetchAll($query); 
	...
	public function addAddress()
	{
		...
		$query 	= "SELECT a.*, store_id  from customer_address_entity a left join customer_entity b on a.parent_id = b.entity_id
					where b.sent_dbm_netsuite = 1 and a.sent_dbm_netsuite = 0 and website_id = 15 order by a.entity_id limit 10"; 
		$result = $this->_getConnection()->fetchAll($query); 	
		...
		foreach ($result as $value) 
		{
			...
			$response = Mage::getModel('webservice/netsuite_postdata_customer')->addAddress($customerId,$addressId,$StoreId);
	...
	public function updateAddress()
	{
		...
		$query 	= "SELECT a.*, store_id  from customer_address_entity a left join customer_entity b on a.parent_id = b.entity_id
					where b.sent_dbm_netsuite = 1 and a.sent_dbm_netsuite = 1 and a.update_senttoNS = 0 and  website_id = 15
		...
		{  
			...
			$reponse = Mage::getModel('webservice/netsuite_postdata_customer')->updateAddress($customerId,$addressId,$StoreId);
		}
		print_r($response);
		return true ;			
	}
	...
	{	
		...
		$query 	= 'SELECT entity_id FROM sales_flat_order WHERE   state != "canceled" and sent_dbm_netsuite = 0  order by entity_id desc limit 10'; 
		$result = $this->_getConnection()->fetchAll($query); 
		
		foreach ($result as $value) 
		{
			$response =Mage::getModel('webservice/netsuite_postdata_order')->createOrder($value['entity_id'],$storeid);
		}
		print_r($response); 
		return true ;
	} 
	...
	{	
		...
		$query 	= "SELECT * FROM  sales_flat_order WHERE state = 'canceled' AND senttoNS = 1 and update_senttoNS = 0 order by entity_id LIMIT 10"; 
		$result = $this->_getConnection()->fetchAll($query); 
		...
		foreach ($result as $value) 
		{
			...
			$response = Mage::getModel('webservice/netsuite_postdata_order')->cancelOrder($value['entity_id'],$StoreId);
