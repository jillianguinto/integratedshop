...
class Unilab_Webservice_Model_Netsuite_Products extends Mage_Core_Model_Abstract
{
	...
	{	
		...
		$this->createlogs( $_POST['sku'],"Magento",$_POST['cmdEvent'],$response['success'],json_encode($response['message']));
			...
			{	
				...
				if($productsku)
				{
					...
					if (isset ( $attributes ['media_gallery'] )) 
					{
						...
						{
							//If image exists
							if ($gallery->getBackend ()->getImage ( $product, $image ['file'] )['file'] == $product->getImage() and $_POST['image_base'] !="") 
							{
								$gallery->getBackend ()->removeImage ( $product, $image ['file'] );
								
							}
							if ($gallery->getBackend ()->getImage ( $product, $image ['file'] )['file'] == $product->getThumbnail() and $_POST['image_thumbnail'] !="") 
							{
								$gallery->getBackend ()->removeImage ( $product, $image ['file'] );
								
							}
							if ($gallery->getBackend ()->getImage ( $product, $image ['file'] )['file'] == $product->getSmallImage() and $_POST['image_small'] !="")
							{
								$gallery->getBackend ()->removeImage ( $product, $image ['file'] );
								
							}
							if ($gallery->getBackend ()->getImage ( $product, $image ['file'] )['file'] != $product->getSmallImage() and $gallery->getBackend ()->getImage ( $product, $image ['file'] )['file'] != $product->getThumbnail() and $gallery->getBackend ()->getImage ( $product, $image ['file'] )['file'] != $product->getThumbnail() and $_POST['images'] !="")
							{
								$gallery->getBackend ()->removeImage ( $product, $image ['file'] );
