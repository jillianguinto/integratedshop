...
class Unilab_Webservice_Model_Validate_Token extends Mage_Core_Model_Abstract
{
	...
	public function validatetoken()
	{
		...
		// Mage::log($token."-".$gettoken."-".$hostname, null, "ref.log");
		// Mage::log("pp", null, "ref.log");
		if (strpos($hostname, 'facebook') !== false) {
			...
			// Mage::log("qq", null, "ref.log");
			$hostname = str_replace("www.","", $hostname);
		}
		
		// Mage::log($referer."-".$hostname, null, "ref.log");
		// if($referer = $_SERVER['HTTP_REFERER'] == ''){
		$qry 	= "SELECT * FROM unilab_token WHERE (token='$token' or token='$gettoken')";
		...
		if($tokenid == "")
		{
			...
			//redirect directly to Teal
			// $storeid 	= Mage::app()->getStore()->getStoreId(); 
			
			// Mage::getSingleton('core/session')->setCategoryId(208);
