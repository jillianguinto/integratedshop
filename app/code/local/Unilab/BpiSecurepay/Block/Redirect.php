...
class Unilab_BpiSecurepay_Block_Redirect extends Mage_Core_Block_Abstract
{
    ...
    { 	 
		...
		if (strlen($config->getConfig()->getMerchantSecureHashSecret()) > 0) {	
			...
			$form->addField('vpc_SecureHash', 'hidden', array('name'=>'vpc_SecureHash', 'value'=>$securhash));
		}
		
		$form->addField('vpc_SecureHashType', 'hidden', array('name'=>'vpc_SecureHashType', 'value'=>'SHA256'));
