 ...
 */
class Magestore_Inventory_Block_Adminhtml_Purchaseorder_Edit_Tab_Form extends Mage_Adminhtml_Block_Widget_Form {
    ...
    protected function _prepareForm() {
        ...
        $fieldset->addField('purchase_on', 'date', array(
            'label' => Mage::helper('inventory')->__('Order Created On'),
        ...
        $fieldset->addField('supplier_id', 'hidden', array(
            'label' => Mage::helper('inventory')->__('Supplier Id'),
            'name' => 'supplier_id',
        ));
        $fieldset->addField('supplier_name', 'link', array(
            ...
            'name' => 'supplier_name',
            'style' => "color:blue",
            'href' => Mage::helper("adminhtml")->getUrl('inventoryadmin/adminhtml_supplier/edit', array('id' => Mage::helper('inventory/purchaseorder')->getDataByPurchaseOrderId($purchaseOrderId, 'supplier_id'))),
            'after_element_html' => $supplierInfo,
        ));

        $fieldset->addField('bill_name', 'text', array(
            'label' => Mage::helper('inventory')->__('Billing Name'),
            'class' => 'required-entry',
            'required' => true,
            'name' => 'bill_name',
        ));
        $fieldset->addField('warehouse_ids', 'label', array(
            'label' => Mage::helper('inventory')->__('Warehouse'),
        ...
        $fieldset->addField('order_placed', 'select', array(
            'label' => Mage::helper('inventory')->__('Order placed via'),
            'class' => 'required-entry',
            'required' => true,
            'name' => 'order_placed',
            'values' => Mage::helper('inventory/purchaseorder')->getOrderPlaced(),
        ));

        $fieldset->addField('start_date', 'date', array(
            'label' => Mage::helper('inventory')->__('Start shipping date'),
            'class' => 'required-entry',
            'required' => true,
            'name' => 'start_date',
            'disabled' => $disabled,
        ...
        $fieldset->addField('cancel_date', 'date', array(
            'label' => Mage::helper('inventory')->__('Cancellation date'),
            'class' => 'required-entry',
            'required' => true,
            'name' => 'cancel_date',
            'disabled' => $disabled,
            ...
            'input_format' => Varien_Date::DATETIME_INTERNAL_FORMAT,
            'format' => Mage::app()->getLocale()->getDateFormat(Mage_Core_Model_Locale::FORMAT_TYPE_SHORT),
            'note' => Mage::helper('inventory')->__('If an "Awaiting delivery" purchase order has no delivery created, you can cancel it before this date.'),
        ));

        $fieldset->addField('expected_date', 'date', array(
            'label' => Mage::helper('inventory')->__('Expected delivery date'),
            'class' => 'required-entry',
            'required' => true,
            'name' => 'expected_date',
            'disabled' => $disabled,
        ...
        $fieldset->addField('payment_date', 'date', array(
            'label' => Mage::helper('inventory')->__('Payment date'),
            'class' => 'required-entry',
            'required' => true,
            'name' => 'payment_date',
            'disabled' => $disabled,
        ...
        $fieldset->addField('ship_via', 'select', array(
            'label' => Mage::helper('inventory')->__('Shipping via'),
            'name' => 'ship_via',
            'values' => Mage::helper('inventory/purchaseorder')->getShippingMethod(),
        ));

        $fieldset->addField('payment_term', 'select', array(
            'label' => Mage::helper('inventory')->__('Payment terms'),
            'name' => 'payment_term',
            'values' => Mage::helper('inventory/purchaseorder')->getPaymentTerms(),
        ));
        ...
        $fieldset->addField('tax_rate', 'text', array(
            'label' => Mage::helper('inventory')->__('Tax Rate'),
            'class' => 'required-entry',
            'required' => true,
            'name' => 'tax_rate',
        ));
        ...
        $fieldset->addField('shipping_cost', 'text', array(
            'label' => Mage::helper('inventory')->__('Shipping Cost'),
            'class' => 'required-entry',
            'required' => true,
            'name' => 'shipping_cost',
...
//            'text' => Mage::app()->getStore($storeId)->getBaseCurrency()->format($data['paid'])
        ...
        $fieldset->addField('paid_more', 'text', array(
            'label' => Mage::helper('inventory')->__('Last paid payment'),
            'required' => false,
            'name' => 'paid_more',
//            'after_element_html' => ' '.$store->getBaseCurrency()->getCode(),  
        ...
        $fieldset->addField('delivery_process', 'label', array(
            'label' => Mage::helper('inventory')->__('Delivery Process'),
            'required' => false,
        ...
        $fieldset->addField('send_mail', 'checkbox', array(
            'label' => Mage::helper('inventory')->__('Send email to supplier'),
            'required' => false,
            'name' => 'send_mail',
...
//            'values'        => Mage::helper('inventory/purchaseorder')->getReturnOrderStatus(),
            ...
            $fieldset->addField('total', 'label', array(
                ...
                'after_element_html' => '
                        <table id="checkout-review-table" class="data-table" style="float:right;">
                            ...
                            <tfoot>
                                <tr class="first">
                                    <td colspan="3" class="a-right" style="padding-right:10px;">' . Mage::helper('inventory')->__('Subtotal') . '</td>
                                ...
                                <tr>
                                    <td colspan="3" class="a-right" style="padding-right:10px;">' . Mage::helper('inventory')->__('Shipping Cost') . '</td>
                                ...
                                <tr>
                                    <td colspan="3" class="a-right" style="padding-right:10px;">' . Mage::helper('inventory')->__('Tax') . '</td>
                                    <td class="a-right last" style=""><span class="price">' . $purchaseOrder->getTaxRate() . '%</span></td>
                                </tr>
                                <tr>
                                    <td colspan="3" class="a-right" style="padding-right:10px;"><strong>' . Mage::helper('inventory')->__('Grand Total (excl. Tax)') . '</strong></td>
                                ...
                                <tr class="last">
                                    <td colspan="3" class="a-right" style="padding-right:10px;"><strong>' . Mage::helper('inventory')->__('Grand Total (incl. Tax)') . '</strong></td>
                                    ...
                                    <!--<br />                                        
                                        <div style="float:right;font-size:14px;">' . Mage::helper('inventory')->__('Subtotal: ') . Mage::getModel('directory/currency')->load($currency)->formatTxt($totalCurrency) . '</div><br />
                                        <div style="float:right;font-size:14px;">' . Mage::helper('inventory')->__('Shipping Cost: ') . Mage::getModel('directory/currency')->load($currency)->formatTxt($shippingCost) . '</div><br />
                                        <div style="float:right;font-size:14px;">' . Mage::helper('inventory')->__('Tax: ') . $purchaseOrder->getTaxRate() . '%</div><br />
                                        <div style="float:right;font-weight:bold;font-size:18px;">' . Mage::helper('inventory')->__('Total: ') . Mage::getModel('directory/currency')->load($currency)->formatTxt($totalCurrency + $shippingCost) . '</div><br />
                                        <div style="float:right;font-weight:bold;font-size:18px;">' . Mage::helper('inventory')->__('Total(incl Tax): ') . Mage::getModel('directory/currency')->load($currency)->formatTxt($totalWithTaxCurrency + $shippingCost) . '</div>
                                            -->
                                        ',
            ));
        }
    ...
    public function getWarehouse($warehouseIds) {
        ...
        foreach ($warehouseIds as $warehouseId) {
            $warehouseNames .= Mage::getModel('inventory/warehouse')->load($warehouseId)->getName() . "<br />";
        }
