...
class Magestore_Inventory_Block_Adminhtml_Report_Sales_Case5 extends Mage_Adminhtml_Block_Widget_Grid {
    ...
    public function __construct() {
        parent::__construct();
        $this->setId('report_period_warehouse');
        $this->setDefaultDir('DESC');
        $this->setDefaultSort('shipmentitem.product_id');
    ...
    protected function _prepareCollection() {
        ...
        $collection->getSelect()
                ...
                ->join(
                    ...
                    ->getCollection()->getTable('sales/shipment_item')), "main_table.shipment_id = shipmentitem.parent_id" .
                        ...
                        " and shipmentitem.order_item_id = main_table.item_id", array('sku', 'name', 'qty', 'price', 'shipmentitem.product_id' => 'shipmentitem.product_id')
                )
                ->join(
        ...
        $collection->getSelect()->group(array('main_table.warehouse_id', 'main_table.product_id'));
    ...
    protected function _prepareColumns() {
        ...
        $this->addExportType('*/*/exportCase5Csv', Mage::helper('adminhtml')->__('CSV'));
        $this->addExportType('*/*/exportCase5Xml', Mage::helper('adminhtml')->__('Excel XML'));
            ...
            switch ($column->getId()) {
                case 'sales_rate':
                    ...
                    if ($column->getDir() == 'asc') {
                        $sorted = usort($arr, array('Magestore_Inventory_Block_Adminhtml_Report_Sales_Case5', 'cmpAscRate'));
                    } else {
                        $sorted = usort($arr, array('Magestore_Inventory_Block_Adminhtml_Report_Sales_Case5', 'cmpDescRate'));
                    }
                    $temp = Mage::helper('inventory/report')->_tempCollection(); // A blank collection 
                    for ($i = 0; $i < count($arr); $i++) {
                        $temp->addItem($arr[$i]);
