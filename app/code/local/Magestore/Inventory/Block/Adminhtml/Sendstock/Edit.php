...
class Magestore_Inventory_Block_Adminhtml_Sendstock_Edit extends Mage_Adminhtml_Block_Widget_Form_Container {
    ...
    public function __construct() {
        ...
        if (!$id) {
            if ($source && $target) {
                $this->_updateButton('back', 'onclick', 'setLocation(\'' . $this->getUrl("inventoryadmin/adminhtml_sendstock/new") . '\')');
                $this->removeButton('save');
                $this->_addButton('checkandsave', array(
                    'name' => 'checkandsave',
                    'label' => Mage::helper('adminhtml')->__('Save Stock Sending'),
            ...
            $this->_updateButton('back', 'onclick', 'setLocation(\'' . $this->getUrl("inventoryadmin/adminhtml_sendstock/index") . '\')');
            $this->removeButton('save');
            $this->removeButton('delete');
            $this->removeButton('reset');
            $sendStock = Mage::getModel('inventory/sendstock')->load($id);
            ...
            if ($warehouseId && Mage::helper('inventory/warehouse')->canTransfer($admin->getId(), $warehouseId)) {
                if ($sendStock->getStatus() == 1) {
                    $day = Mage::getStoreConfig('inventory/transaction/cancel_time');
        ...
        $this->_formScripts[] = "
            ...
            function saveAndContinueEditCheck(){
                ...
                if((!checkProduct) || (checkProduct=='')){
                    ...
                    var check_product_url = '" . $this->getUrl('inventoryadmin/adminhtml_sendstock/checkproduct') . "';
...
//                editForm.submit();
            ...
            function cancelSending(){
                if (confirm('Are you sure you want to cancel this stock sending? The Qty. sent will be instantly added to the total Qty. in your source warehouse.')) { 
                    var url = '" . $this->getUrl('inventory/adminhtml_sendstock/cancel', array('id' => $this->getRequest()->getParam('id'))) . "';
            ...
            function uploadFile() {
                ...
                xhr.open('POST', '" . $this->getUrl('inventoryadmin/adminhtml_sendstock/getImportCsv', array('source' => $source, 'target' => $target)) . "');
                xhr.send(fd);
           }
    ...
    public function getHeaderText() {
        if (Mage::registry('sendstock_data') && Mage::registry('sendstock_data')->getId()
            ...
            if (Mage::registry('sendstock_data')->getStatus() == 2) {
                return Mage::helper('inventory')->__("View Stock Sending No.'%s'", $this->htmlEscape(Mage::helper('inventory')->getIncrementId(Mage::registry('sendstock_data')))
                );
            } else {
                return Mage::helper('inventory')->__("Edit Stock Sending No.'%s'", $this->htmlEscape(Mage::helper('inventory')->getIncrementId(Mage::registry('sendstock_data')))
                );
            }
        }
        return Mage::helper('inventory')->__('New Stock Sending');
    }
