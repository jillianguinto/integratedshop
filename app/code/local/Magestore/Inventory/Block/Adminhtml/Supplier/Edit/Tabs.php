 ...
 */
class Magestore_Inventory_Block_Adminhtml_Supplier_Edit_Tabs extends Mage_Adminhtml_Block_Widget_Tabs {

    public function __construct() {
        parent::__construct();
        $this->setId('supplier_tabs');
        $this->setDestElementId('edit_form');
        $this->setTitle(Mage::helper('inventory')->__('Supplier Information'));
    }
    ...
    protected function _beforeToHtml() {
        $this->addTab('form_section', array(
            ...
            'content' => $this->getLayout()
                    ->createBlock('inventory/adminhtml_supplier_edit_tab_form')
                    ->toHtml(),
        ));
        ...
        if ($this->getRequest()->getParam('id')) {
            ...
            if (Mage::getStoreConfig('inventory/dropship/enable')) {
                $this->addTab('dropship_section', array(
                    'label' => Mage::helper('inventory')->__('Drop Shipments'),
                    'title' => Mage::helper('inventory')->__('Drop Shipments'),
                    'url' => $this->getUrl('inventorydropshipadmin/adminhtml_inventorydropship/supplierdropship', array(
                        '_current' => true,
                        'id' => $this->getRequest()->getParam('id'),
                        'store' => $this->getRequest()->getParam('store')
                    )),
            ...
            $this->addTab('history_section', array(
                ...
                'content' => $this->getLayout()
                        ->createBlock('inventory/adminhtml_supplier_edit_tab_history')
                        ->toHtml(),
            ));
        }
