 ...
 */
class Magestore_Inventory_Adminhtml_PaymenttermController extends Mage_Adminhtml_Controller_Action
{
    /**
     * init layout and set active for current menu
     *
     * @return Magestore_Inventory_Adminhtml_PaymenttermController 
     */
    protected function _initAction()
    {
        $this->loadLayout()
            ->_setActiveMenu('inventory/paymentterm')
            ->_addBreadcrumb(
    ...
    public function editAction()
    {
        ...
        $model  = Mage::getModel('inventory/paymentterm')->load($paymentTermId);
        ...
        if ($model->getId() || $paymentTermId == 0) {
            ...
            $this->loadLayout();
            $this->_setActiveMenu('inventory/paymentterm');
            ...
            $this->_addContent($this->getLayout()->createBlock('inventory/adminhtml_paymentterm_edit'))
                ->_addLeft($this->getLayout()->createBlock('inventory/adminhtml_paymentterm_edit_tabs'));

            $this->renderLayout();
        } else {
            Mage::getSingleton('adminhtml/session')->addError(
                Mage::helper('inventory')->__('Payment term does not exist')
            );
            $this->_redirect('*/*/');
        }
    }
     ...
     public function saveAction()
    {
        if ($data = $this->getRequest()->getPost()) {        
            $model = Mage::getModel('inventory/paymentterm');        
            if($this->getRequest()->getParam('id')){
//                $model = Mage::getModel('inventory/paymentterm')->load($this->getRequest()->getParam('id'));
            ...
            try {
                if($this->getRequest()->getParam('id')){
                    $oldData = Mage::getModel('inventory/paymentterm')->load($this->getRequest()->getParam('id'));
                    $changeArray = array();
                    $changeData = 0;
                ...
                $model->save();
                if(!$this->getRequest()->getParam('id')){
                    $paymentTermHistory = Mage::getModel('inventory/paymenttermhistory');
                    $paymentTermHistoryContent = Mage::getModel('inventory/paymenttermhistorycontent');
                    ...
                    $paymentTermHistoryContent->setData('payment_term_history_id',$paymentTermHistory->getId())
                                              ->setData('field_name',$admin.' created this payment term!')
                                              ->save();
                }else{
                    if($changeData == 1){
                        $paymentTermHistory = Mage::getModel('inventory/paymenttermhistory');
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('inventory')->__('Payment Term was successfully saved!')
                );
        ...
        Mage::getSingleton('adminhtml/session')->addError(
            Mage::helper('inventory')->__('Unable to find payment term to save!')
        );
        $this->_redirect('*/*/');
    }
    ...
    {
        $fieldArray = array(
                            'name'=>Mage::helper('inventory')->__('Payment term name'),
                            'description'=>Mage::helper('inventory')->__('Description'),
                            'status' => Mage::helper('inventory')->__('Status'),
                            'payment_days' => Mage::helper('inventory')->__('Payment day(s)'),
    ...
    {
        return array('name','description','payment_days','status');
    }
    ...
    {
        $paymentTermIds = $this->getRequest()->getParam('inventory');
        if (!is_array($paymentTermIds)) {
            ...
            try {
                foreach ($paymentTermIds as $paymentTermId) {
                    $supplier = Mage::getModel('inventory/paymentterm')->load($paymentTermId);
                    $supplier->delete();
                }
                Mage::getSingleton('adminhtml/session')->addSuccess(
    ...
    public function massStatusAction()
    {
        ...
        if (!is_array($paymentTermIds)) {
            ...
            try {
                foreach ($paymentTermIds as $paymentTermId) {
                    Mage::getSingleton('inventory/paymentterm')
                        ->load($paymentTermId)
    ...
    public function exportCsvAction()
    {
        ...
        $content    = $this->getLayout()
                           ->createBlock('inventory/adminhtml_paymentterm_grid')
                           ->getCsv();
    ...
    public function historyAction() {
        $this->loadLayout();
	$this->getLayout()->getBlock('inventory.paymentterm.edit.tab.history');
        $this->renderLayout();
    }	
    ...
    public function showhistoryAction() {
        $form_html = $this->getLayout()
            ->createBlock('inventory/adminhtml_paymentterm')
            ->setTemplate('inventory/paymentterm/showhistory.phtml')
            ->toHtml();
