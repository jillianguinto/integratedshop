...
/**
 * Assignment Resource Collection Model
 * 
 * @category    Magestore
 * @package     Magestore_Inventory
 * @author      Magestore Developer
 */
class Magestore_Inventory_Model_Mysql4_Assignment_Collection extends Mage_Core_Model_Mysql4_Collection_Abstract
{
    ...
    /**
     * get admin id that can transfer stock of warehouse
     * @return array of admin id
     */
    public function getAllCanTransferAdmins(){
        $idsSelect = clone $this->getSelect();
        $idsSelect->reset(Zend_Db_Select::ORDER);
        $idsSelect->reset(Zend_Db_Select::LIMIT_COUNT);
        $idsSelect->reset(Zend_Db_Select::LIMIT_OFFSET);
        $idsSelect->reset(Zend_Db_Select::COLUMNS);
        $idsSelect->where('main_table.can_transfer=?',1);
        $idsSelect->columns('main_table.admin_id');
        $idsSelect->resetJoinLeft();
        return $this->getConnection()->fetchCol($idsSelect);
    }
