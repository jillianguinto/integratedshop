...
class RRA_Checkout_ShippingrateController extends Mage_Core_Controller_Front_Action
{
    ...
    {  
		try{		
			...
			// $grandT 	= Mage::getSingleton('checkout/cart')->getQuote()->collectTotals();
			// $grandTotal = Mage::helper('core')->currency($grandT->getSubtotal(), true, false);	
			
			$grandT 	= Mage::getSingleton('checkout/cart')->getQuote()->collectTotals()->getSubtotal(); 
			$grandTotal = $grandT;
			
			$sessioncart = Mage::getModel('checkout/session')->getQuote();
			$discountTotal = 0;
			foreach ($sessioncart->getAllItems() as $item){
			...
			$grandTotal							= ($grandTotal-$discountTotal) + $shippingrate;
			...
			if($carrier_method == "notserviceable"){
