...
class Clean_SqlReports_Block_Adminhtml_Report_View_Grid extends Mage_Adminhtml_Block_Widget_Grid
{
    ...
    protected function _createCollection()
    {
		...
		if($where != "")		{			$where = " WHERE ".$where;		}else{			$where = "";		}								$report_id     = $this->getRequest()->getParam('report_id');				if($report_id== "9" || $report_id== "10")		{			$sql = $sql[0];		}else{			$sql = $sql[0].$where." GROUP BY ".$sql[1];		}
