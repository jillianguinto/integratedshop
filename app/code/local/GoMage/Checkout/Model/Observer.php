...
class GoMage_Checkout_Model_Observer {
	...
	public function hookToSaveOrderAfterEvent($event){

		

		if( $order = Mage::getModel('sales/order')->loadByIncrementId($event->getOrder()->getIncrementId()) ){
			...
			try{
				...
				**/
				// if( in_array( $order->getPayment()->getMethod() , array('ipay88','bpisecurepay','rcbcpay')) ){
				//if( in_array( $order->getPayment()->getMethod() , array('ipay88','bpisecurepay','rcbcpay','perapal')) ){
				if( in_array( $order->getPayment()->getMethod() , array('ipay88','bpisecurepay','rcbcpay', 'bankdeposit', 'perapal')) ){ 

					$orderid = $event->getOrder()->getIncrementId();
					$payment_method_code = $order->getPayment()->getMethod();
					$set_log = $orderid." - ".$payment_method_code;
					//Mage::log($set_log, null, 'paymentmethod-sendemail.log');
					
					$order->sendNewOrderEmail();

					//file_put_contents('./debug.txt', 'sending email'.PHP_EOL,FILE_APPEND);
	 ...
	 * Check if all items in checkout page are valid with prescription if product requires prescription

	 * @added : diszo.saisl (2014-01-14)

	 */



	public function validateOnepageItems($event) {
		...
		file_put_contents('./debug.txt', 'validateOnepageItems'.PHP_EOL,FILE_APPEND);
		...
		if ($h->getConfigData('general/disable_cart') && $h->getConfigData('general/enabled') && $h->isAvailableWebsite()) {
			...
			if ($quote->hasItems()) {
					...
					if($item->getProduct()->getTypeId() == 'simple') {
						...
						if (!empty($parentIdsArray) && isset($parentIdsArray[0])) {
							...
							elseif($__product->getUnilabRx() && $item->getPrescriptionId() != "" && !Mage::helper('prescription')->isValidPrescription($item->getPrescriptionId())){
								...
								$item->addErrorInfo('prescription','RX',Mage::helper('prescription')->__('Invalid attached prescription.'));
				...
				if($has_error){
					...
					$event->getEvent()->getControllerAction()->getResponse()->setRedirect(Mage::getUrl('checkout/cart'));
