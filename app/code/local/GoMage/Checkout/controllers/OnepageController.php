...
class GoMage_Checkout_OnepageController extends Mage_Checkout_Controller_Action {
	...
	public function indexAction() {
		...
		$this->__initCartItems();
		
		/*
		 * Redirect to login form if not login  
	...
	public function ajaxAction() {
		...
		//ADDED TO HANDLE SHIPPING RATE RECALL  - January 17, 2014
		$calculator->result->shipping_recall = false; 
		$shipping_groups = $this->getOnepage()->getQuote()->getShippingAddress()->getGroupedAllShippingRates();					
		if(count($shipping_groups) > 0){
			$shipping_codes = array_keys($shipping_groups);
			if(!in_array('xend',$shipping_codes)){
				$calculator->result->shipping_recall = true; 
			} 
		}else{ 
			$calculator->result->shipping_recall     = true;
		}				
		...
		$calculator->result->current_shipping_method = $this->getOnepage()->getQuote()->getShippingAddress()->getShippingMethod(); 
	...
	protected function __initCartItems(){
		...
		foreach($quote->getAllItems() as $item){					
			...
			// Expected simple product in cart items
			if($item->getProduct()->getTypeId() == 'simple') {
				$parent = Mage::getModel('catalog/product_type_configurable');
						$parentIdsArray = $parent->getParentIdsByChild( $item->getProduct()->getId() );
						if (!empty($parentIdsArray) && isset($parentIdsArray[0])) {
					        
   
					    	$__product = Mage::getModel('catalog/product')
			        					->setStoreId(Mage::app()->getStore()->getId())
			        					->load($parentIdsArray[0]);
			        
					if($__product->getUnilabRx() && $item->getPrescriptionId() == ""){
						//$item->addErrorInfo('checkout','RX',Mage::helper('prescription')->__('This product requires prescription.'));
						$errorItems[] = $item->getProduct()->getName(). ' (SKU: '.$item->getProduct()->getSku().')';
						$has_error = true; 
					}
					elseif($__product->getUnilabRx() && $item->getPrescriptionId() != "" && !Mage::helper('prescription')->isValidPrescription($item->getPrescriptionId())){
						//$item->addErrorInfo('checkout','RX',Mage::helper('prescription')->__('Invalid attached prescription.'));
						$errorItems[] = $item->getProduct()->getName(). ' (SKU: '.$item->getProduct()->getSku().')';
						$has_error = true;
					}
			    }
		...
		if($has_error && count($errorItems)>0){
			$erMessage = Mage::helper('prescription')->__('One or more product(s) require(s) a prescription. Please remove affected item(s) or add the item(s) manually to proceed with checkout:').'</br>'.implode("</br>", $errorItems);
			Mage::getSingleton('checkout/session')->addError($erMessage);
			session_write_close();
			$this->_redirect('checkout/cart');
            return;
		}
