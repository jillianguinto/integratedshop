...
abstract class Mage_Payment_Model_Method_Abstract extends Varien_Object
{
   ...
   /**
    ...
    public function isApplicableToQuote($quote, $checksBitMask)
    {
        ...
        if ($checksBitMask & self::CHECK_ORDER_TOTAL_MIN_MAX) {
            ...
            if (!empty($minTotal) && $total < $minTotal || !empty($maxTotal) && $total > $maxTotal) {
                return false;
            }elseif($this->checkvalidlocation() == true && $this->getConfigData('codename') === 'xendcod'){				               return false;						}
        }
    ...
    }		protected function checkvalidlocation(){				$storeId = Mage::app()->getStore()->getStoreId();						$allowallregion 	= Mage::getStoreConfig('payment/cashondelivery/allowallregion',$storeId);		$allowallcities 	= Mage::getStoreConfig('payment/cashondelivery/allowallcities',$storeId);		$sallowallregion	= false;		$sallowallcities	= false;		$isvalid			= false;				$citiesAllowed 		= $this->_getCitiesAvailable();		$currentCity		= $this->_getcustomercity();					if($allowallcities == 0){			if (in_array($currentCity, explode(",",$citiesAllowed))) {								$sallowallcities	= true;							}					}else{						$sallowallcities= true;			}						if($allowallregion == 0){						$regionallowed 	= $this->_getRegionAvailable();			$currentRegion	= $this->_getcustomerregion();							if (in_array($currentRegion, explode(",",$regionallowed))) {								$sallowallregion	= true;				}				}else{			$sallowallregion	= true;					}							if($sallowallregion == false){						$isvalid = true;						}elseif($sallowallregion == true && $sallowallcities == false){						$isvalid = true;						}					return $isvalid;			}			protected function _getcustomerregion(){	  		$quote 				= Mage::getSingleton('checkout/session')->getQuote();		$customerAddressId	= $quote->getShippingAddress()->getregion_id();		if ($customerAddressId){						return	$customerAddressId;				}			return;	}	protected function _getcustomercity(){	  		$quote 			= Mage::getSingleton('checkout/session')->getQuote();		$customercity	= $quote->getShippingAddress()->getcity();		return	$customercity;			}			protected function _getRegionAvailable(){		return Mage::getStoreConfig('payment/cashondelivery/specificregion');	  	}  	protected function _getCitiesAvailable(){		return Mage::getStoreConfig('payment/cashondelivery/specificcities');	  	} 			
}
