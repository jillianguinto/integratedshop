        ...
        if ($data) {
            ...
            try {
                ...
                if ($address) {
                  ...
                  //Modified: Pushing Associate Website ID into store_id in customer_entity when added from backend
                  $store_id =  $customer->getStoreId();
                 if($store_id == 0 ){
                     $websiteId =  $customer->getWebsiteId();
                    $query  = "SELECT store_id FROM core_store WHERE website_id='$websiteId'"; 
                ...
                if ($isNewCustomer) {
                    ...
                    $adddressids                = $read->fetchAll("SELECT entity_id FROM `customer_address_entity` WHERE `parent_id` = $customerId and senttoNS = 0 "); 
                    ...
                    Mage::getModel('webservice/netsuite_postdata_customer')->sendCustomerWithoutAddress($customerId,$storeidapi);
                        if ($address){

                                if($addressId == ""){ 
                                 foreach ($adddressids as $key => $value) {
                                   
                                        $customerId = $customer->getId();
                                        $newAddress = $this->getRequest()->getPost();
                            ...
                            if($netsuiteApiEnabled == 1 ){    
                                Mage::getModel('webservice/netsuite_postdata_customer')->updateCustomer($customerId,$storeidapi); 
                                   ...
                                   if ($address){
                                    ...
                                    if($addressId == "" || $adddressids != '' || $adddressids != null){ 
                                            foreach ($adddressids as $key => $value) 
                                            {
                                             ...
                                             foreach ($modifiedAddresses as $key => $value)
                                            {
