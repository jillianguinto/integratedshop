...
class Mage_Adminhtml_Block_Promo_Quote_Edit_Tab_Coupons_Grid extends Mage_Adminhtml_Block_Widget_Grid
{
	...
	public function getMainButtonsHtml()			{					$id = $this->getRequest()->getParam('id');				$addButton = null;			$html = parent::getMainButtonsHtml();					$addButton .= $this->getLayout()->createBlock('adminhtml/widget_button')->setData(array(							'label'     => Mage::helper('adminhtml')->__('Import Coupon Code'),							'onclick'   => "setLocation('".$this->getUrl("admincontroller/adminhtml_addcouponcode/index/id/$id")."')",							'class'   => 'scalable add'						))->toHtml();								return $addButton.$html;				}	
    /**
     * Get grid url
     *
     * @return string
