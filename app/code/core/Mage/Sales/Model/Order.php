...

{
    ...
    public function sendNewOrderEmail()
    {
        ...
        $paymentcode = $this->getStatus();
		
		$orderid = $this->getId();
		
			$read = Mage::getSingleton('core/resource')->getConnection('core_write');	
		
			$qry 	= "select count(prescription_id) as is_rx from sales_flat_order_item where order_id='$orderid'";						
			$rs 	= $read->fetchRow($qry); 
			
		$is_rx = $rs['is_rx'];
        
        if($paymentcode == "cashondelivery" || $paymentcode == "perapal" || $paymentcode == "bankdeposit")
        {
            if($is_rx != 0)
			{
				$is_rx = "Our pharmacist will contact you for prescription verification.";
			}else{
				$is_rx = "";
			}
        ...
        if ($this->getCustomerIsGuest()) {
            ...
            }elseif($paymentcode == "perapal"){
                
                $templateId = Mage::getStoreConfig('payment/perapal/email_template'); //perapal
				
			}elseif($paymentcode == "bankdeposit"){
                
                $templateId = Mage::getStoreConfig('payment/bankdeposit/email_template'); //bank deposit
            }else{
        ...
        if ($copyTo && $copyMethod == 'bcc') {
            ...
            foreach ($copyTo as $email) 
            {
                ...
                // $mailer->addEmailInfo($emailInfo); //binaba para iwas multiple send
            }
        ...
        if($paymentcode == "cashondelivery")
        {
            $copyTo = Mage::getStoreConfig('payment/cashondelivery/email_recipient'); 
            $email_to_cc = explode(',', $copyTo);
                
            foreach ($email_to_cc as $email) 
            {
                $emailInfo = Mage::getModel('core/email_info');
                $emailInfo->addTo($email); 
        ...
        {
            $copyTo = Mage::getStoreConfig('payment/bankdeposit/email_recipient'); //bank deposit
            $email_to_cc = explode(',', $copyTo);
    ...
    public function sendNewOrderEmail2()
    {
        $storeId = $this->getStore()->getId();
