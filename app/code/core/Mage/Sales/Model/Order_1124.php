...
class Mage_Sales_Model_Order extends Mage_Sales_Model_Abstract
{
     ...
     * 小omplete and closed states are encapsulated intentionally, see the _checkState()
     *
     * @param string $state
     * @param string|bool $status
     * @param string $comment
    ...
    public function sendNewOrderEmail()
    {
        ...
        $payment_method_codes =     $this->getPayment()->getMethodInstance()->getTitle();
        $payment_method_code   =    $this->getPayment()->getMethodInstance()->getCode();
        // $code =  Mage::getSingleton('core/session')->getpaymentcode(); 
...
if($code == "cashondelivery" || $code == "perapal"):

        Mage::log($code, null, 'sendNewOrderEmail-'.$code.'.log');
        ...
        if ($this->getCustomerIsGuest()) {
            ...
            // Mage::getSingleton('core/session')->setwhatwhat($what);



        } else {
            
            // $payment_method_code =   $this->getPayment()->getMethodInstance()->getTitle();


            //payment method $payment_method_code = Mage::getSingleton('checkout/session')->getQuote()->getPayment()->getMethodInstance()->getCode();
            //kapag COD 'cashondelivery'
            if($code == "cashondelivery")
            {
                $templateId = Mage::getStoreConfig('payment/cashondelivery/email_template'); // module ng COD,s
            }   
            // elseif($payment_method_code == "Cebuana Lhuillier")
            elseif($code == "perapal")
            {
                $templateId = Mage::getStoreConfig('payment/perapal/email_template'); //perapal
            }
            else
            {
        ...
        if ($copyTo && $copyMethod == 'bcc') {
            ...
            if($code == "cashondelivery"):
                $copyTo = Mage::getStoreConfig('payment/cashondelivery/email_recipient'); // module ng COD,
                $email_to_cc = explode(',', $copyTo);

                $emailInfo->addBcc($email_to_cc);
            elseif($code == "perapal"):
                $copyTo = Mage::getStoreConfig('payment/perapal/email_recipient'); //perapal
                $email_to_cc = explode(',', $copyTo);
