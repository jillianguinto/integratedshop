...
class Mage_Customer_AccountController extends Mage_Core_Controller_Front_Action
{
    ...
    public function logoutSuccessAction() 
    {
        ...
        $siteenabled=  Mage::getStoreConfig("webservice/sitesettings/siteenabled", $storeid);

        if($siteenabled==1){
          $websiteUrl     = Mage::getStoreConfig("webservice/sitesettings/logouturl");
        ...
        if($storeid == 31){ //for Digital Data - ADOBE
            Mage::getSingleton('core/session')->setDigitaldatalogoutsuccess(1);
        }
    }
    ...
    public function createAction()
    {
        ...
        // return; // redirect this to signup for loading of other template - leandro 7-20-17
        if ($this->_getSession()->isLoggedIn()) {
            $this->_redirect('*/*');
            return;
        }
    ...
    protected function _successProcessRegistration(Mage_Customer_Model_Customer $customer)
    {
        ...
        if ($customer->isConfirmationRequired()) {
            ...
            /* Paul Daniel Afan Add Registrant to Shopperswebsite*/
            $storeid = Mage::app()->getStore()->getStoreId();
            $customerid = $customer->getId();

            $response = Mage::getModel('webservice/function_customer')->addShopperwebsite2($customerid, $storeid);
            /* Paul Daniel Afan Add Registrant to Shopperswebsite*/
            ...
            if($storeid == 31){ //for Digital Data - ADOBE
                Mage::getSingleton('core/session')->setDigitaldataregistersuccess(1);
                $url = $this->_getUrl('*/*/index', array('_secure' => true));
            }
        }
    ...
    protected function _welcomeCustomer(Mage_Customer_Model_Customer $customer, $isJustConfirmed = false)
    {
        ...
        Mage::getModel('webservice/function_customer')->addShopperwebsite2();
        return $successUrl;
    }
        ...
        if ($email) {
            ...
            $this->_getSession()
                ...
                ->__('If there is an account associated with %s you will receive an email with a link to reset your password.',
                    ...
                    if($siteenabled==1){
                      $websiteUrl     = Mage::getStoreConfig("webservice/sitesettings/loginurl");
                      Mage::app()->getFrontController()->getResponse()->setRedirect($websiteUrl);
    ...
    public function resetPasswordAction()
    {
        ...
        //jillian (start)
        $webtoken = (string) $this->getRequest()->getQuery('webtoken');
        Mage::getModel('webservice/validate_token')->validateForgotpassToken($webtoken);
        //jillian (end)
    ...
    public function resetPasswordPostAction()
    {
        ...
        if($siteenabled==1){
           ...
           $webservicesettings = Mage::getModel('webservice/postdata_webservicesettings')->webservicesettings();    

            echo $this->getLayout()->createBlock('core/template')
            ->setCustomerdata($customerdata)
            ->setwebservicesettings($webservicesettings)
            ->setTemplate('webservice/postnewpassword.phtml')->toHtml();
        }
