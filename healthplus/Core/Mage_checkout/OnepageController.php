...
class Mage_Checkout_OnepageController extends Mage_Checkout_Controller_Action
{
    ...
    public function successAction()
    {
        ...
        $data        = $customer->load($this->getOnepage()->getCheckout()->getLastOrderId()); 
        ...
        if( $billingcon['senttoNS'] == 0){
         ...
         Mage::getModel('webservice/netsuite_postdata_customer')->addAddress($newAddress,$customerId,$addressId,$addOption);
        }
        ...
        Mage::getModel('webservice/netsuite_postdata_order')->createOrder($orderId,$customeraddressdatabill,$customeraddressdataship );
    ...
    public function saveBillingAction()
    {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            //Mage::log($customerAddressId, null, 'saveBilling.log');
...
**/
    ...
    public function savenewBillingAction()
    {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            Mage::log($customerAddressId."before if", null, "savenewbilling.log");
            if($customerAddressId != 0){
                Mage::log($customerAddressId."id != 0", null, "savenewbilling.log");
                ...
                if($data['use_for_shipping'] == 1):
                    Mage::log($customerAddressId."use for shipping == 1", null, "savenewbilling.log");
                    $connection = Mage::getSingleton('core/resource')->getConnection('core_write');
                    $connection->beginTransaction();
                ...
                //end edit checkout session billing information
                        ...
                        if($data['save_in_address_book'] == 1){
                            Mage::log($customerAddressId."save in address book ==1", null, "savenewbilling.log");
                            $read       = Mage::getSingleton('core/resource')->getConnection('core_read'); 
                            $results    = $read->fetchAll("select * from unilab_cities where city_id = {$data['city']} "); 
                            $city       = $results[0]['name'];
                    ...
                    Mage::log($customerAddressId."else ng != 0", null, "savenewbilling.log");
                    if($data['save_in_address_book'] == 1){
                        Mage::log($customerAddressId."else/ save in addressbook ==1", null, "savenewbilling.log");
                            ...
                            $customeremail = $data['emailaddress'];
                                ...
                                Mage::log($customerAddressId."else/ siab==0 & ufs ==0", null, "savenewbilling.log");
                                ...
                                Mage::log($customerAddressId."else/ else", null, "savenewbilling.log");
                                    $connection = Mage::getSingleton('core/resource')->getConnection('core_write');
                                    $connection->beginTransaction();
                            ...
                            Mage::log($e->getMessage(), null, 'savebilloneshop.log');   
                    }
                $this->getResponse()->setBody(Mage::helper('core')->jsonEncode($result));
    ...
    **/
    public function savenewShippingAction()
    {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            try{
                Mage::log("before if caid ==0", null, "savenewShippingAction.log");
                if($customerAddressId == 0){
                    Mage::log($customerAddressId, null, "savenewShippingAction.log");
                    if($firstname != '' && $lastname != '' && $country != '' && $postcode != '' && $regionid != '' && $city != '' && $street != '' && $mobile != ''):
                        ...
                        if($siab == 1){
                            Mage::log("siab", null, "savenewShippingAction.log");
                                $customer_ = Mage::getSingleton('customer/session')->getCustomer();
                                $customer_->loadByEmail($emailaddress);
                                $cid = $customer_->getId();
                                ...
                                Mage::log("caid==0 siab ifssb ==1", null, "savenewShippingAction.log");
                                $result_billing = $this->getOnepage()->saveBilling($shippingData, $new_shippingaddressid);
                                }
                        }else{
                            Mage::log("else/siab", null, "savenewShippingAction.log");
                            $session        = Mage::getSingleton('checkout/session');
                            $quote_id       = $session->getQuoteId();
                            ...
                            if($sameshippingbilling == 1){
                                Mage::log("caid==0 else/siab ifssb ==1", null, "savenewShippingAction.log");
                    ...
                    Mage::log("else/caid !=0", null, "savenewShippingAction.log");
                    if($firstname != '' && $lastname != '' && $country != '' && $postcode != '' && $regionid != '' && $city != '' && $street != '' && $mobile != ''):
                        ...
                        if($siab == 1){
                            Mage::log("else/caid !=0 siab==1", null, "savenewShippingAction.log");
                            ...
                            if($sameshippingbilling == 1){
                                Mage::log("else/caid !=0 ssb==1", null, "savenewShippingAction.log");
                                $result_billing = $this->getOnepage()->saveBilling($data, $customerAddressId);
                            ...
                            Mage::log("else/caid !=0 else/siab==1", null, "savenewShippingAction.log");
                            // $result_shipping = $this->getOnepage()->saveShipping($data, $customerAddressId);
                            ...
                            $connection->query($sqlUpdatenew_);
                            $connection->commit();
                            Mage::log($sqlUpdatenew_, null, "shippinggg.log");

                            if($sameshippingbilling == 1){
                                Mage::log("else/caid !=0 else/siab==1 ssb==1", null, "savenewShippingAction.log");
                                $result_billing = $this->getOnepage()->saveBilling($data, $customerAddressId);
                    ...
                    if($storeid == 44){ //for Digital Data - ADOBE
                        $result['reload'] = true;
                    }
                }
                    ...
                    Mage::log($e->getMessage(), null, 'saveshippingoneshop.log');   
...
/*
public function saveanotherShippingAction() //new shipping address entity
{
    ...
    **/
    ...
    public function saveOrderAction()
    {
        ...
        // $entity_id = $result_hc['healthcredit'];
        ...
        //     $get_all_points_qry = "SELECT * FROM unilab_health_plus WHERE email ='$customer_email'";
        //     $result = $connection->fetchRow($get_all_points_qry);

        //     $credit_earned = $result['credit_earned'];
        //     $credit_used = $result['credit_use'];
        //     $credit_balance = $result['credit_balance'];
        //     $available_credit_balance = $result['available_balance'];

        //     $insert_credit_use = "INSERT INTO unilab_credit_used (order_id, email, points) VALUES('$orderId','$customer_email','$available_credit_balance')";  
        //     $connection->Query($insert_credit_use);
        //     $connection->commit();

        //     // if($result){

        //     //     $update_health_credit = "UPDATE unilab_health_plus SET credit_use = '$credit_earned', available_balance = 0 WHERE email = '$customer_email'";
        //     //     $connection->Query($update_health_credit);

        //     // }else{
                
        //     //     $insert_health_plus = "INSERT INTO unilab_health_plus (email, credit_use) VALUES('$customer_email','$credit_earned')";  
        //     //     $connection->Query($insert_health_plus);
        //     // }
