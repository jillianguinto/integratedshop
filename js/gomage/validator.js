...
Object.extend(Validation, {
    ...
    test : function(name, elm, useTitle) {
        ...
        if(Validation.isVisible(elm) && !v.test($F(elm), elm)) {
			...
			/** OVENT CUSTOM **/			
			if(elm.id == 'billing_mobile' && $("billing_telephone").value !=""){
				var advice1 = Validation.getAdvice(name, elm);
				this.hideAdvice(elm, advice1);
				this.hideAdvice($("billing_telephone"), advice1);
				this.updateCallback($("billing_telephone"), 'passed');
				return true;
			}			
			if(elm.id == 'billing_telephone' && $("billing_mobile").value !=""){
				var advice1 = Validation.getAdvice(name, elm);
				this.hideAdvice(elm, advice1);
				this.hideAdvice($("billing_mobile"), advice1);
				this.updateCallback($("billing_mobile"), 'passed');
				return true;
			}			
			if(elm.id == 'shipping_mobile' && $("shipping_telephone").value !=""){
				var advice1 = Validation.getAdvice(name, elm);
				this.hideAdvice(elm, advice);
				this.hideAdvice($("shipping_telephone"), advice1);
				this.updateCallback($("shipping_telephone"), 'passed');
				return true;
			}			
			if(elm.id == 'shipping_telephone' && $("shipping_mobile").value !=""){
				var advice1 = Validation.getAdvice(name, elm);
				this.hideAdvice(elm, advice1);
				this.hideAdvice($("shipping_mobile"), advice1);
				this.updateCallback($("shipping_mobile"), 'passed');
				return true;
			}			
