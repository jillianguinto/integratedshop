...
class GoMage_Checkout_Block_Sales_Order_Creditmemo_Totals extends Mage_Sales_Block_Order_Creditmemo_Totals
{
	protected function _initTotals()
    {
		...
		$order = Mage::getModel('sales/order')->load($this->getSource()->getOrderId());		
		if($order->getId() && strpos( $order->getShippingMethod() , 'xend_') !== false ){
			/* $creditmemo = new Varien_Object(array(
					'code'		=>	'fee',				
					'value'     => 	$this->getSource()->getFeeAmount(),
	                'base_value'=> 	$this->getSource()->getBaseFeeAmount(),
	                'label'		=>	Mage::helper('fee')->__('Insurance')
			));
	                
			$this->addTotalBefore($creditmemo, 'grand_total'); */
