...
class GoMage_Checkout_Model_Quote_Address extends Mage_Sales_Model_Quote_Address
{
	...
	public function __construct(){
		...
		$this->required_fields = array(
			...
			'mobile'		=>array('NotEmpty', 'Please enter the mobile number.'),
	...
	public function validate(){
        ...
        foreach($this->required_fields as $fieldName=>$method){
			...
			if($fieldName == 'mobile' && Zend_Validate::is($this->getData('telephone'), $method[0])){
				continue;			
			}elseif($fieldName == 'telephone' && Zend_Validate::is($this->getData('mobile'), $method[0])){
				continue;	
			}
