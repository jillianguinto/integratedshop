<?php
	...
	{ 
		if(count($results) > 0 ){  
			...
			$filename  = $_SERVER['DOCUMENT_ROOT'].'/exportcsvOptimized/csv/'.$name.'.csv';
	...
	function sendtoftp($name) 
	{
		...
		$file = $_SERVER['DOCUMENT_ROOT'].'/exportcsvOptimized/csv/'.$name;
		$remote_file = 'cdmsOptimization/csv/' . $ftpfolder . '/'.$name;
	...
	function import_to_dbm($name)
	{
		...
		$ch = curl_init('http://161.202.200.30/cdmsOptimization/dbm/api/integimport/' . $name . '/'); 
