...
class Customermod extends CI_Model {
    ...
    function getadminuserstoreid($website_ids){
    	...
    	$sql1 			= "SELECT name FROM core_website WHERE website_id='$website_ids'";
		$sqlResult1 		= $this->db->query($sql1); 
	...
	function getWebsiteIds($website_ids){
		$sql = "SELECT name FROM core_store WHERE website_id = '{$website_ids}'";
		$sqlResult 		= $this->db->query($sql); 		
		return $sqlResult->result();
	}
	...
	function getExportCust($website_ids){
		...
		$sql = "SELECT 
				...
				WHERE customer_entity.website_id = '$website_ids'";
		$sqlResult 		= $this->db->query($sql); 
		return $sqlResult->result();
	}
	...
	{	
		...
		$sql = "SELECT 	customer_entity.entity_id, 
				...
				-- INNER JOIN log_customer 	
				-- ON log_customer.customer_id = customer_entity.entity_id
				-- AND log_customer.store_id = '$website_ids'
